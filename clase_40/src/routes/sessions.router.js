...
router.get('/auth', middleware.checkAuth, (req,res) =>{
    ServerResponse.success({req:req, res, data:{userID:req.user._id, cartID:req.user.cartID, email:req.user.email}})
})
...
router.post('/login', passport.authenticate('login',{failureRedirect:'/api/sessions/loginfail'}), async (req,res)=>{
    req.session.user = {id:req.user._id, name:req.user.name, lastname:req.user.lastname, email:req.user.email, role:req.user.role, avatar:req.user.avatar, cartID:req.user.cartID}
